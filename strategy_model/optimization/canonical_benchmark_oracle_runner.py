        ...
        rows.append(
            {
                ...
                "config": "100 MW / 10 h CAES, RTE 0.55 discharge-side, 249 MW grid cap",
...
def main() -> None:
    ...
    out = args.out or (repo / "strategy_model" / "optimization" / "canonical_benchmarks_2020")
    ...
    commands = [
        f"{sys.executable} {Path(__file__).resolve()} --repo {repo} --out {out} --horizons {' '.join(map(str, args.horizons))}",
    ]
    ...
    write_registry(out, summaries, repo, commit)
